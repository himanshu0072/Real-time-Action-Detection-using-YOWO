...
labelmap, _ = read_labelmap(r"usr\datasets\AVA\annotations\ava_action_list_v2.2.pbtxt")
...
while (cap.isOpened()):
    ...
    frame = img = cv2.resize(
        frame, (crop_size, crop_size), interpolation=cv2.INTER_LINEAR)
    ...
    with torch.no_grad():
        ...
        preds = []
        all_boxes = get_region_boxes_ava(
            output, conf_thresh_valid, num_classes, anchors, num_anchors, 0, 1)
        ...
        if len(scores) > 0:
            ...
            for _, cls_ind in enumerate(indices):
                text.append("[{:.2f}] ".format(scores[_]) +
                            str(labelmap[cls_ind]['name']))
                text_size.append(cv2.getTextSize(
                    text[-1], font, fontScale=0.25, thickness=1)[0])
